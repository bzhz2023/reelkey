...
Webhook 用于接收 Creem 的支付状态通知。ReelKey 当前的 BYOK 买断产品通过
`checkout.completed` 事件写入 `byok_entitlements`，授予 lifetime access。

> 重要：BYOK 买断履约不要依赖 Better Auth Creem 插件的默认 webhook 回调。
> 线上实测中插件回调能打印 `onCheckoutCompleted CALLED`，但后续写库可能没有可靠完成。
> 生产环境应使用项目里的自定义 webhook 路由
> `src/app/api/auth/creem/webhook/route.ts`，该路由会在返回 200 前 await 写库。
...
```yaml
Endpoint URL: https://reelkey.app/api/auth/creem/webhook
Events (选择以下事件):
  - checkout.completed ✅ (BYOK 买断支付完成，必须)
  - refund.created ✅ (退款撤权，建议保留)
  - dispute.created ✅ (争议撤权，建议保留)
...
如果在 Creem 后台重新创建了 webhook endpoint，必须把该 endpoint 页面里的
Signing secret 同步到 Vercel Production 的 `CREEM_WEBHOOK_SECRET`，然后重新部署。
`{"error":"Invalid signature"}` 基本就是该 secret 不匹配。

### 步骤 3：验证 Webhook

Creem 的 **Send test event** 只能验证 endpoint 连通和签名是否正确。测试事件通常使用
Creem 示例 product，且 `metadata` 可能为空，因此不能证明 BYOK lifetime 授权会落库。

```bash
# 查看线上日志
vercel logs --environment production --since 15m --limit 1000 --no-branch --expand

# 验证授权表
pnpm exec dotenv -e .env.local -- node -e "const postgres=require('postgres');(async()=>{const sql=postgres(process.env.DATABASE_URL,{max:1});console.log(await sql\`select user_id,status,product_id,order_id,checkout_id,metadata,created_at from byok_entitlements order by created_at desc limit 5\`);await sql.end();})()"
```

完整验证必须使用真实 checkout 或在 Creem Dashboard 重放真实的
`checkout.completed` 事件。有效事件应满足：

- `product.id` 等于 `NEXT_PUBLIC_CREEM_LIFETIME_EARLY_BIRD_PRODUCT_ID` 或 `NEXT_PUBLIC_CREEM_LIFETIME_REGULAR_PRODUCT_ID`
- `object.metadata.referenceId` 是 ReelKey 用户 ID
- 线上日志出现 `BYOK lifetime entitlement granted`
- `byok_entitlements` 新增或更新为 `status = active`
...
4. 检查服务器日志，确认不是 `Invalid signature`
5. 注意：test event 不能验证 BYOK 授权落库，因为它通常没有真实 product ID 和 `metadata.referenceId`

要验证授权落库，请完成一次真实测试支付，或重放真实 `checkout.completed` 事件：

```sql
SELECT user_id, status, product_id, order_id, checkout_id, metadata, created_at
FROM byok_entitlements
...
### Q3: 支付成功但 BYOK 买断没有生效？

**原因**：Webhook 未正确处理，或事件不是 BYOK lifetime 产品，或缺少 `metadata.referenceId`。

**调试步骤**：
```bash
# 1. 查看线上日志
vercel logs --environment production --since 30m --limit 1000 --no-branch --expand
...
- `Invalid signature`：Creem endpoint 的 Signing secret 与 `CREEM_WEBHOOK_SECRET` 不一致。
- `metadata=undefined`：通常是 Send test event，不会写入 BYOK 授权。
- `checkout.completed is not a BYOK lifetime product`：事件 product ID 不是配置的买断产品。
- 有真实 product ID 和 `referenceId`，但没有落库：优先确认是否仍命中 Better Auth catch-all，而不是自定义 route。

...
export default function DemoPage() {
    ...
    const pollStatus = async () => {
      try {
        ...
        if (result.status === "COMPLETED") {
          ...
          toast({ title: "Error", description: result.error || "Video generation failed", variant: "destructive" });
          setIsGenerating(false);
          refreshCredits();
        }
      } catch (error) {
  ...
  const handleSubmit = async (data: SubmitData) => {
    ...
    if (data.type !== "video") {
      toast({ title: "Error", description: "Only video generation is supported", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      refreshCredits();
      toast({ title: "Started", description: `Video generation started! ${result.creditsUsed} credits frozen.` });
    } catch (error) {
